...
def intent_node(state: RoutingState, emit: Emitter) -> RoutingState:
    ...
    if state.selected_dept == "cardiology":
        dept_name = "Cardiology"
        doc_name = CARDIOLOGY_DOCTOR_NAME
    elif state.selected_dept == "neurology":
        dept_name = "Neurology"
        doc_name = NEUROLOGY_DOCTOR_NAME
    ...
    reply = _llm_reply(
        RECEPTIONIST_PERSONA + (
            ...
            f"Recommend booking a slot with {doc_name}, and explain that they can see options for all of our available doctors to choose from. "
    ...
    if state.health_question_round == 2:
        ...
        reply = _llm_reply(
            RECEPTIONIST_PERSONA + (
                ...
                f"Recommend booking with {doc_name}, but mention they can select from any of our available doctors. "
...
def _persist_consultation_context(state: RoutingState, appointment_id: str | None, doctor_name: str | None, department: str | None) -> None:
    try:
        from backend.db.pgvector_tracker import upsert_consultation_context
    except ImportError:
...
def booking_node(state: RoutingState, emit: Emitter) -> RoutingState:
    ...
    if appointment_client is not None:
        try:
            ...
            # Map doctor ID: "d5" -> 1, "d8" -> 2, "d9" -> 3
            go_doctor_id = 1
            if state.selected_doctor == "d5":
                go_doctor_id = 1
            elif state.selected_doctor == "d8":
                go_doctor_id = 2
            elif state.selected_doctor == "d9":
                go_doctor_id = 3

            # Map slot ID: "s_gp" -> 1, "s_cardio" -> 2, "s_neuro" -> 3
            go_slot_id = 1
            if state.selected_slot == "s_gp":
                go_slot_id = 1
            elif state.selected_slot == "s_cardio":
                go_slot_id = 2
            elif state.selected_slot == "s_neuro":
                go_slot_id = 3

            # Call appointment service
            try:
                resp = appointment_client.book(go_doctor_id, go_user_id or 0, go_slot_id)
