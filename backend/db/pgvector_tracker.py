    ...
    with _conn() as conn:
       ...
       """)
        ...
        try:
            ...
            if cur.fetchone()[0] == 0:
                cur.execute("INSERT INTO departments (id, name) VALUES (1, 'General Physician'), (2, 'Cardiology'), (3, 'Neurology') ON CONFLICT (id) DO NOTHING")
            
            cur.execute("SELECT COUNT(*) FROM doctors")
            if cur.fetchone()[0] == 0:
                cur.execute("""
                    INSERT INTO doctors (id, department_id, name, specialty) VALUES 
                    (1, 1, 'Dr. Shankar Dada', 'General Physician'),
                    (2, 2, 'Dr. Arjun Reddy', 'Cardiology'),
                    (3, 3, 'Dr. Octopus', 'Neurology')
            ...
            if cur.fetchone()[0] == 0:
                cur.execute("""
                    ...
                    (1, 1, NOW() + INTERVAL '1 hour', NOW() + INTERVAL '2 hours', true),
                    (2, 2, NOW() + INTERVAL '2 hours', NOW() + INTERVAL '3 hours', true),
                    (3, 3, NOW() + INTERVAL '3 hours', NOW() + INTERVAL '4 hours', true)
                    ON CONFLICT (id) DO NOTHING
                """)
...
def _generate_patient_id() -> str:
    ...
    with _conn() as conn:
        ...
        cur = conn.cursor()
        # Use RIGHT(id, 6) to get the last 6 digits from IDs like PAT-2026-000010
        cur.execute("SELECT COALESCE(MAX(CAST(RIGHT(id, 6) AS INTEGER)), 0) FROM users WHERE id LIKE %s", (prefix + '%',))
        row = cur.fetchone()
        cur.close()
        try:
