...
Usage:
    python -m backend.cardiology.ingest.embed_store <chunks.jsonl> <department>
"""
...
try:
    from backend.cardiology.db.pgvector_tracker import (  # noqa: E402
        count_knowledge_chunks,
        init_db,
        insert_knowledge_chunks,
    )
    from backend.cardiology.llm.embeddings import embed_passages  # noqa: E402
except ImportError:
...
def main() -> int:
    if len(sys.argv) != 3:
        print("usage: python -m backend.cardiology.ingest.embed_store <chunks.jsonl> <department>")
        return 2
