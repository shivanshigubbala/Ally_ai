...
try:
    ...
    from backend.cardiology.models.session_state import ChatRequest, ChatResponse
    from backend.cardiology.ws.router import router as ws_router
    from backend.cardiology.db.pgvector_tracker import init_db, seed_default_user
...
logging.getLogger("backend.cardiology.graphs").setLevel(logging.INFO)
logging.getLogger("backend.cardiology.ws").setLevel(logging.INFO)
logging.getLogger("backend.cardiology.llm").setLevel(logging.INFO)
...
async def upload_document(
    ...
    """
    try:
        from backend.cardiology.ws.router import _doc_store
    except ImportError:
    ...
    file_id = None
    try:
        from backend.cardiology.db.pgvector_tracker import (
            create_session,
    ...
    try:
        try:
            from backend.cardiology.ws.router import notify_user_event
            from backend.cardiology.models.session_state import WSEvent
        except ImportError:
    ...
    if chunks:
        try:
            try:
                from backend.cardiology.llm.embeddings import embed_passages
                from backend.cardiology.db.pgvector_tracker import insert_knowledge_chunks
            except ImportError:
...
async def internal_report_ready(payload: dict = Body(...)) -> dict:
    ...
    try:
        ...
        try:
            from backend.cardiology.db.pgvector_tracker import create_notification
        except Exception:
