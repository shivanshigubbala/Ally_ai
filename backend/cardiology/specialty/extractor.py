...
class PatientInformationExtractor:
    """Extract structured medical information from the latest patient message."""

    def update_state(self, state: CardiologyState, patient_message: str) -> CardiologyState:
        ...
        for symptom in symptom_keywords:
            if symptom in message and symptom not in state.symptoms:
        ...
        duration_pattern = re.search(r"(\d+)\s*(minute|minutes|hour|hours|day|days|week|weeks)", message)
        if duration_pattern:
            state.duration = duration_pattern.group()

        severity_pattern = re.search(r"\b([1-9]|10)\b", message)
        if severity_pattern:
        ...
        for symptom in associated:
            if symptom in message and symptom not in state.associated_symptoms:
        ...
        for disease in history:
            if disease in message and disease not in state.medical_history:
        ...
        if "smoker" in message or "smoking" in message or "i smoke" in message:
            state.smoking = True
