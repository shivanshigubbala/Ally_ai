...
from backend.cardiology.agent import session_complete, questioning

class TestCardiologyMemory(unittest.TestCase):
    @patch("backend.cardiology.agent.upsert_clinical_profile")
    @patch("backend.cardiology.agent._call_llm")
    def test_longitudinal_memory_cardiology(self, mock_call_llm, mock_upsert_profile):
        # 1. Visit 1
        state_v1 = DoctorState(
            ...
            appointment_id="101",
            doctor_id="cardiology_physician",
            consultation_context_id="ctx-101",
            doctor_name="Dr. Meera Rao",
            department="cardiology",
            chief_complaint="chest tightness",
            tests_list=[{"name": "ECG", "reason": "Check rhythm"}],
            lab_request_status="PENDING_LAB"
        )
        state_v1.consultation_summary = {
            "clinical_assessment": "Cardiology assessment performed.",
            "possible_diagnosis": "Angina suspected",
        ...
        self.assertEqual(updates_arg["department"], "cardiology")
        
        visit_entry = updates_arg["visit_entry"]
        self.assertEqual(visit_entry["appointment_id"], "101")
        self.assertEqual(visit_entry["doctor_name"], "Dr. Meera Rao")
        self.assertEqual(visit_entry["chief_complaint"], "chest tightness")
        ...
        state_v2 = DoctorState(
            ...
            doctor_name="Dr. Meera Rao",
            department="cardiology",
            chief_complaint="tightness follow-up",
        ...
        mock_profile = {
            ...
            "last_visit_per_department": {
                "cardiology": visit_entry
            }
        }
        
        with patch("backend.cardiology.agent.get_clinical_profile", return_value=mock_profile) as mock_get_profile:
            with patch("backend.cardiology.agent._stream_into_emit") as mock_stream:
                ...
                self.assertIn("LAST VISIT WITH THIS DEPARTMENT (cardiology)", system_prompt)
                self.assertIn("Tests recommended: ECG (status: PENDING_LAB)", system_prompt)
