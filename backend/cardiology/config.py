...
def _find_by_name_fragment(fragment: str) -> Optional[dict]:
    try:
        docs = store.list_doctors("cardiology")
    except Exception:
        return None
    frag = fragment.lower()
    for d in docs:
...
def get_default_doctor_id() -> str:
    env = os.getenv("ALLY_CARDIOLOGY_DOCTOR_ID")
    if env:
        return env
    d = _find_by_name_fragment("arjun")
    if d:
        return d.get("id") or "d8"
    return "d8"


def get_default_doctor_name() -> str:
    env = os.getenv("ALLY_CARDIOLOGY_DOCTOR_NAME")
    if env:
        return env
    d = _find_by_name_fragment("arjun")
    if d:
        return d.get("name") or "Dr. Arjun Reddy"
