...
from backend.general_physician.agent import session_complete, questioning

class TestGeneralPhysicianMemory(unittest.TestCase):
    @patch("backend.general_physician.agent.upsert_clinical_profile")
    @patch("backend.general_physician.agent._call_llm")
    def test_longitudinal_memory_gp(self, mock_call_llm, mock_upsert_profile):
        # 1. Visit 1
        state_v1 = DoctorState(
            ...
            appointment_id="101",
            doctor_id="general_physician",
            consultation_context_id="ctx-101",
            doctor_name="Dr. Shankar",
            department="general",
            chief_complaint="persistent cough",
            tests_list=[{"name": "Complete Blood Count (CBC)", "reason": "Check infection"}],
            lab_request_status="PENDING_LAB"
        )
        state_v1.consultation_summary = {
            "clinical_assessment": "Mild upper respiratory tract infection suspected.",
            "possible_diagnosis": "Mild URTI",
        ...
        # Assert that upsert_clinical_profile was called with extracted facts and visit entry
        ...
        self.assertEqual(updates_arg["department"], "general")
        
        visit_entry = updates_arg["visit_entry"]
        self.assertEqual(visit_entry["appointment_id"], "101")
        self.assertEqual(visit_entry["doctor_name"], "Dr. Shankar")
        self.assertEqual(visit_entry["chief_complaint"], "persistent cough")
        ...
        state_v2 = DoctorState(
            ...
            doctor_name="Dr. Shankar",
            department="general",
            chief_complaint="fever follow-up",
        ...
        with patch("backend.general_physician.agent.get_clinical_profile", return_value=mock_profile) as mock_get_profile:
            with patch("backend.general_physician.agent._stream_into_emit") as mock_stream:
                ...
                self.assertIn("LAST VISIT WITH THIS DEPARTMENT (general)", system_prompt)
                self.assertIn("Tests recommended: Complete Blood Count (CBC) (status: PENDING_LAB)", system_prompt)
