...
def session_init(state: DoctorState, emit: Emitter) -> DoctorState:
    ...
    if state.uploaded_documents:
        ...
        try:
            ...
            logger.exception("Failed to stream initial GP doctor message with documents, falling back to static/non-stream")
        ...
        try:
            if isinstance(lab_response, dict) and lab_response.get("reports"):
                for r in lab_response.get("reports"):
                    ...
                    # IMPORTANT: appointment_id and session_id are required so
                    # the frontend can clear the "pending-{sessionId}" card.
                    emit(WSEvent(type="report_ready", payload={
                        "report_id": report_id,
                        "report_url": path,
                        "download_url": path,
                        "pdf_name": pdf_name,
                        "appointment_id": str(state.appointment_id or ""),
                        "session_id": str(state.appointment_id or ""),
                        "tests": state.tests_list,
                        "doctor": state.doctor_name or DOCTOR_NAME,
