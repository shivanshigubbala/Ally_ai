...
try:
    from backend.neurology.db.pgvector_tracker import save_message
    from backend.neurology.llm.nvidia_client import chat as nv_chat, ROUTING_MODEL
    from backend.neurology.models.session_state import RoutingState, WSEvent
    from backend.neurology.services import local_store as store
...
def _persist_consultation_context(state: RoutingState, appointment_id: str | None, doctor_name: str | None, department: str | None) -> None:
    try:
        from backend.neurology.db.pgvector_tracker import upsert_consultation_context
    except ImportError:
...
def booking_node(state: RoutingState, emit: Emitter) -> RoutingState:
    ...
    if appointment_client is not None:
        try:
            ...
            go_user_id = None
            try:
                from backend.neurology.db.pgvector_tracker import sync_go_user_id
            except Exception:
                try:
